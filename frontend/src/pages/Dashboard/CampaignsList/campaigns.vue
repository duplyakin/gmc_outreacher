        ...
        >
          ...
          >
            <template slot-scope="scope">
              <a
                @click.prevent="editCampaign(scope.row, scope.$index)"
                href="#"
          ...
          <el-table-column :min-width="50" fixed="right" label="Action">
            <template slot-scope="props">
              <a
                ...
                href="#"
              >{{ props.row.status == 1 ? 'Pause' : 'Start' }}</a>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    swal_make_action(campaign, index, path) {
      swal
        .fire({
          title: "Are you sure?",
          text: "Campaign status will be changed!",
          icon: "warning",
    ...
    make_action(campaign, index) {
      ...
      //this.swal_make_action(campaign, path);
    ...
    editCampaign(msg_dict, index) {
      ...
      this.$router.push({
        ...
        query: { campaign_id: msg_dict._id.$oid }
      });
    },
    next_page() {
      var page = 2;
