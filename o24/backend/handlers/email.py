...
CHECK_BOUNCED_ACTION_HANDLERS = {
    'smtp' : gmail_api.check_bounced,  # FOR GMAIL credentials we also check replies via API
    'api' : gmail_api.check_bounced
}
...
@celery.task
def email_check_bounced(task_id):
    result_data = {
        'if_true' : False,
        'code' : -1,
        'error' : 'Unknown Error'
    ...
    try:
        task = shared.TaskQueue.lock(task_id)
        if not task:
            print("CONCURRENCY in email_check_bounced attempt")
            return 
        ...
        params['credentials_id'] = task.credentials_id
        if not params['credentials_id']:
            raise Exception("email_check_bounced ERROR: credentials_id can't be None for task_id:{0}".format(task_id))
        ...
        handler = CHECK_BOUNCED_ACTION_HANDLERS.get(sender, None)
        if not handler:
            message = "email_check_bounced: Unsupported email sender={0}".format(sender)
            raise Exception(message)
        ...
        if task:
            unlocked = shared.TaskQueue.unlock(task_id=task_id, result_data=result_data, status=status)
            if not unlocked:
                raise Exception("Can't unlock email_check_bounced")

            #log task
            scheduler_models.ActionLog.log(task, step='handler', description="email_check_bounced")
        return result_data
